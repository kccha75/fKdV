...
function [v,gamma]=pseudoarclength_pre2(v,gamma,dv,dgamma,ds,steps,...
...
%           disp(rms(r2))
        ...
        delta_lambda=(-P-psi1*dot(dv,cellz1{1}))/(psi2*dgamma-psi1*dot(dv,cellz2{1}));
    ...
    elseif converged==true
        ...
        dgamma1=1/(psi2*dgamma-psi1*dot(dv,cellz2{1}));
        dv1=-psi2*dgamma*cellz2{1};
        ...
        if abs(psi1*dot(dv,dv1)+psi2*dgamma*dgamma1)<0.7 && j>3
            
            j=j-1;
            converged=false;
            fprintf('gradient change too large!\n')
            % Do not save latest vectors if not converged
            v(:,j+1)=[];
            gamma(:,j+1)=[];
        ...
        elseif psi1*dot(dv,dv1)+psi2*dgamma*dgamma1<0
